...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "/checkin — start inventory\n"
        "/find [number] — find part\n"
        "/audit [query] — search\n"
        "/report — summary\n"
        "/export — Excel\n"
        "/progress — progress",
...
async def _do_search(message: Message, state: FSMContext, query: str):
    ...
    if similar:
        ...
        kb.button(text="❌ None", callback_data="ci_cancel")
        kb.adjust(1)
    ...
    kb.button(text="📸 Send label photo", callback_data="ci_photo")
    kb.adjust(1)
    await message.answer(
...
async def cb_similar(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Similar parts:", reply_markup=kb.as_markup())
...
async def handle_qty(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<i>e.g. <code>A-04-B-2</code> or <code>PALLET-08</code></i>",
        parse_mode="HTML",
    )
...
async def handle_cond(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "📸 Take a photo of the part or box.\n"
...
async def handle_checkin_photo(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"ID: <b>{part.get('inventory_id', '')}</b> | Location: <b>{location}</b>\n"
        f"{quantity} units × {price_s} = <b>{total_s}</b>\n"
        f"Condition: {condition} | Photo: ✅\n"
        f"{'DB: ✅' if sheets_ok else 'DB: ⚠️ error'}\n\n"
        "/checkin — next part",
        parse_mode="HTML",
    )
...
async def skip_photo(message: Message, state: FSMContext):
    if message.text.strip().lower() in ("skip", "-", "no"):
        data = await state.get_data()
        await state.clear()
        part = data["part"]
        update_part_in_db(
